 ...
 *
 * Integrates WebSocket real-time updates with the enhanced room store.
...
export function useRoomWebSocket(options: {
  ...
  // Store selectors
  const updateDeviceStatus = useRoomStore(state => state.updateDeviceStatus);
  const updateRoomAggregateStatus = useRoomStore(state => state.updateRoomAggregateStatus);
