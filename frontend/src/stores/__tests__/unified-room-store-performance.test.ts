/**
 * Performance and Caching Tests for Unified Room Store
 * 
...
// Mock the API client
vi.mock('@/lib/api/unified-room-api-client', () => ({
  unifiedRoomApiClient: {
    getAllRooms: vi.fn(),
    getRoomById: vi.fn(),
    controlDevice: vi.fn(),
    clearCache: vi.fn(),
...
describe('Unified Room Store - Performance & Caching', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    localStorageMock.getItem.mockReturnValue(null);
    
    // Reset store state
    useUnifiedRoomStore.getState().clearCache();
  });

  afterEach(() => {
    // Cleanup intervals
    useUnifiedRoomStore.getState().disableBackgroundRefresh();
  });

  describe('Caching', () => {
    it('should cache room data and serve from cache on subsequent requests', async () => {
      const mockGetAllRooms = vi.mocked(unifiedRoomApiClient.getAllRooms);
  ...
  describe('Request Deduplication', () => {
    it('should deduplicate concurrent requests for the same room', async () => {
      const mockGetRoomById = vi.mocked(unifiedRoomApiClient.getRoomById);
  ...
  describe('Optimistic Updates', () => {
    it('should apply optimistic updates immediately', async () => {
      const mockControlDevice = vi.mocked(unifiedRoomApiClient.controlDevice);
  ...
  describe('Cache Management', () => {
    ...
    it('should clear all cache data', async () => {
      ...
      expect(localStorageMock.removeItem).toHaveBeenCalledWith('unified_room_cache');
    });
  });
});