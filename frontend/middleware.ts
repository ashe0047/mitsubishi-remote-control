...
const publicAccessRoutes = ["/offline", "/api/auth/refresh", "/api/health"];

// Admin-only routes (for parent users)
const adminRoutes = ["/family", "/quotas/manage", "/dashboard/admin"];

// Child-only routes
const childRoutes = ["/quotas/status", "/dashboard/child"];
